...
from shutil import move, copyfile, copyfileobj, SameFileError
...
def pathlike_move_safe(
    src: PurePath | bytes, dst: PurePath, copy: bool = False
) -> PurePath:
    ...
    try:
        ...
        # shutil throws SameFileError, w/e
        return dstp
    return dstp
...
def check_path_dupes(path: PurePath) -> PurePath:
    if not (Path(path).is_file() and Path(path).stat().st_size):
        return path
    c = len(
        [file for file in Path(path.parent).iterdir() if file.match(path.stem + "*")]
    )
...
def bulk_regex_urls(urls: str | list[str]) -> list[list[str]]:
    ...
    base_url = (
        r"(?:https?://)?open\."
        + base_uri.split(":")[1]
        + r"\.com(?:/intl-\w+)?/%s/([0-9a-zA-Z]{22})(?:\?si=.+?)?"
    )
    ...
    for i, req_type in enumerate(ITEM_NAMES):
        matched_ids[i] = re.findall(base_uri % req_type, urls) + re.findall(
            base_url % req_type, urls
        )
    return matched_ids
...
def edge_zip(sorted_list: list) -> None:
    ...
    n = len(sorted_list)
    sorted_list[::2], sorted_list[1::2] = (
        sorted_list[: (n + 1) // 2],
        sorted_list[: (n + 1) // 2 - 1 : -1],
    )
...
def select(
    ...
    return [
        items[i - first_ID]
        for i in (indices[:1] if only_one else indices)
        if i - first_ID >= 0
    ]
...
def conv_artist_format(artists: list, FORCE_NO_LIST: bool = False) -> list[str] | str:
    ...
    artist_names = (
        [a.name for a in artists] if isinstance(artists[0], Artist) else artists
    )
    if Zotify.CONFIG.get_artist_delimiter() == "":
...
def strptime_utc(dtstr) -> datetime.datetime:
    return datetime.datetime.strptime(dtstr[:-1], "%Y-%m-%dT%H:%M:%S").replace(
...
def wait_between_downloads(skip_wait: bool = False) -> None:
    ...
    if waittime > 5:
        Printer.hashtaged(
            ...
            f"PAUSED: WAITING FOR {waittime} SECONDS BETWEEN DOWNLOADS",
        )
    time.sleep(waittime)
...
def upgrade_legacy_archive(entries: list[str], archive_path: PurePath) -> None:
    ...
    for i, entry in enumerate(entries):
        ...
        for glob_path in Path(Zotify.CONFIG.get_root_path()).glob(
            "**/" + str(filename_or_path)
        ):
            reliable_tags, unreliable_tags = Track.parse_audio_tags(PurePath(glob_path))
            if (
                "trackid" in unreliable_tags
                and unreliable_tags["trackid"] == entry_items[0]
...
def add_obj_to_song_archive(obj, dir_path: PurePath | None = None) -> None:
    ...
    add_to_archive(obj.id, "", author_name, item_name, obj.filepath, archive_path, mode)


# Playlist File Utils
def add_to_m3u8(
    m3u8_path: PurePath, contents: list, append_strs: list[str] | None = None
):
    ...
    with open(m3u8_path, "a", encoding="utf-8") as file:
        for content in contents:
            ...
            track_label_m3u = (
                f"#EXTINF:{content.duration_ms // 1000}, {content.printing_label}\n"
            )
